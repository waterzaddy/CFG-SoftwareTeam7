...
def scheduled_decay():
    decay_rate = 0.1
    pet.health = max(pet.min_status, pet.health - decay_rate)
    pet.happiness = max(pet.min_status, pet.happiness - decay_rate)
...
def index():
    if "user" in session:
        ...
        if "last_login" in session:
            ...
            # Calculate slower decay based on elapsed time and apply to pet's health and happiness
            decay_rate_between_sessions = 0.05  # can be adjusted
            decay_amount = decay_rate_between_sessions * (elapsed_time.total_seconds() / (60 * 60 * 24))  # Assuming 1 day
            pet.health = max(pet.min_status, pet.health - decay_amount)
            pet.happiness = max(pet.min_status, pet.happiness - decay_amount)

            # Enforce lower limits for health and happiness
            pet.health = max(pet.min_status, pet.health)
            pet.happiness = max(pet.min_status, pet.happiness)

            # Update last login time
            session["last_login"] = current_time

        return render_template("index.html", todos_health=todos_health,
                               todos_happiness=todos_happiness, pet=pet, user=user)
    else:
